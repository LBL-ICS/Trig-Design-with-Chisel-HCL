...
%  * 
y_input_file = fopen('rtl-theta-input.txt', 'w');
theta_output_file = fopen('rtl-cos-output.txt', 'w');
for i= 1:10000
        ...
        rand_y=mnval+rand*(mxval-mnval)
        y_hex=dec2hex(typecast(single(rand_y), 'uint32'))
         
        fprintf(y_input_file, '%08s\n', y_hex);
        
        theta_output=aran(rand_y)
